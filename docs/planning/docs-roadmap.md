...
| [`build-and-publish.md`](../build-and-publish.md) | `build.gradle`, `settings.gradle`, `gradle.properties`, `.github/workflows/*.yml` | Gradle build configuration, GitHub Actions CI/CD workflows (CI, snapshot, release), Maven Central publishing of `hoist-core` and `hoist-core-mcp` via Sonatype Central Portal, GPG artifact signing, release validation and versioning, Toolbox `repository_dispatch` via the XH Build Bot GitHub App, required GitHub secrets and variables, `maven-archive.xh.io` static archive for legacy `36.x`-and-earlier releases | Done |
